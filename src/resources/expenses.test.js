...
test('expenses lifecycle', () => {
  const repository = new ExpensesRepository();
  const created = repository.create({ name: 'Example', projectId: 'value', category: 'value', amount: 1, approved: true });
...
test('expenses input validation', () => {
  const repository = new ExpensesRepository();
