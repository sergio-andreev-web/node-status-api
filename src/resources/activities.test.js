...
test('activities lifecycle', () => {
  const repository = new ActivitiesRepository();
  const created = repository.create({ name: 'Example', actorId: 'value', subjectId: 'value', sequence: 1, visible: true });
...
test('activities input validation', () => {
  const repository = new ActivitiesRepository();
