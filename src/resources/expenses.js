...
function validate(input, partial = false) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new TypeError('expenses input must be an object');
  }
  const result = {};
  for (const key of Object.keys(input)) {
    if (!fieldNames.includes(key)) throw new TypeError(`Unknown expenses field: ${key}`);
...
export const resourceName = 'expenses';
