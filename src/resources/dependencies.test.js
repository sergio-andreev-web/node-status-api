...
test('dependencies lifecycle', () => {
  const repository = new DependenciesRepository();
  const created = repository.create({ name: 'Example', taskId: 'value', dependsOnId: 'value', lagDays: 1, resolved: true });
...
test('dependencies input validation', () => {
  const repository = new DependenciesRepository();
