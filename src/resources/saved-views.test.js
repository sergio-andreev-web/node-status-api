...
test('savedViews lifecycle', () => {
  const repository = new SavedViewsRepository();
  const created = repository.create({ name: 'Example', ownerId: 'value', query: 'value', uses: 1, public: true });
...
test('savedViews input validation', () => {
  const repository = new SavedViewsRepository();
