...
test('releases lifecycle', () => {
  const repository = new ReleasesRepository();
  const created = repository.create({ name: 'Example', projectId: 'value', version: 'value', build: 1, published: true });
...
test('releases input validation', () => {
  const repository = new ReleasesRepository();
