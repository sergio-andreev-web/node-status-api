...
test('notifications lifecycle', () => {
  const repository = new NotificationsRepository();
  const created = repository.create({ name: 'Example', userId: 'value', channel: 'value', attempts: 1, read: true });
...
test('notifications input validation', () => {
  const repository = new NotificationsRepository();
