...
test('invitations lifecycle', () => {
  const repository = new InvitationsRepository();
  const created = repository.create({ name: 'Example', email: 'value', teamId: 'value', attempts: 1, accepted: true });
...
test('invitations input validation', () => {
  const repository = new InvitationsRepository();
