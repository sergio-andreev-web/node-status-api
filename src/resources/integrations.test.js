...
test('integrations lifecycle', () => {
  const repository = new IntegrationsRepository();
  const created = repository.create({ name: 'Example', projectId: 'value', provider: 'value', syncCount: 1, active: true });
...
test('integrations input validation', () => {
  const repository = new IntegrationsRepository();
