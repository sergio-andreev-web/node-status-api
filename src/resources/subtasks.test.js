...
test('subtasks lifecycle', () => {
  const repository = new SubtasksRepository();
  const created = repository.create({ name: 'Example', taskId: 'value', assigneeId: 'value', estimate: 1, done: true });
...
test('subtasks input validation', () => {
  const repository = new SubtasksRepository();
